...
class DraftAgent:
    ...
    def get_next_pick_info(self, round_number):
        """Informa cuando es el proximo pick de Mauro."""
        current_pos = self.get_pick_position(round_number)
        next_pos = self.get_pick_position(round_number + 1)
        
        if current_pos == 3:
            # Elige penultimo en esta ronda, segundo en la siguiente
            picks_until_next = (4 - current_pos) + next_pos  # 1 + 2 = 3 picks
        else:
            # Elige segundo en esta ronda, penultimo en la siguiente
            picks_until_next = (4 - current_pos) + next_pos  # 2 + 3 = 5 picks
    ...
    def get_roster_needs(self):
        ...
        total_starters = sum(v for k, v in ROSTER_SLOTS.items() if k != "BENCH" and k != "FLEX")
        bench_needed = ROSTER_SLOTS["BENCH"]
        bench_current = max(0, len(self.my_roster) - sum(counts.values()))

        if bench_current < bench_needed:
            needs.append(f"BENCH - {bench_current}/{bench_needed} slots cubiertos")
    ...
    def recommend(self, round_number):
        ...
        JUGADORES DISPONIBLES POR POSICION:
        QBs: {', '.join(available_by_pos.get('QB', [])[:15])}
        RBs: {', '.join(available_by_pos.get('RB', [])[:20])}
        WRs: {', '.join(available_by_pos.get('WR', [])[:20])}
        TEs: {', '.join(available_by_pos.get('TE', [])[:15])}
        Ks: {', '.join(available_by_pos.get('K', [])[:10])}
        DEFs: {', '.join(available_by_pos.get('DEF', [])[:10])}

        INSTRUCCIONES:
        1. Considera cuantos picks hay hasta el proximo turno de Mauro
        2. Evalua si un jugador top puede desaparecer antes de su proximo pick
        3. Considera las necesidades del roster pero sin sacrificar valor en rondas tempranas
        4. En rondas tardias (10+) prioriza cubrir starters faltantes y banca util
        5. NO recomendar los QBs reservados para los amigos
        6. Si Mauro aun no tiene QB y {my_qb} esta disponible y es momento correcto, incluirlo
        ...
        system_prompt = "Eres un experto en NFL fantasy football draft strategy. Das recomendaciones precisas y adaptadas al contexto del roster y el draft serpentina."
